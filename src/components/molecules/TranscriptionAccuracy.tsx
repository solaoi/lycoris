...
const TranscriptionAccuracy = (): JSX.Element => {
    ...
    const downloadedModelsReazonSpeech = useRecoilValue(modelReazonSpeechDownloadedState)
        ...
        <div className="dropdown">
            {((isRecording || isTracing) ||
                ...
                (downloadedModels.length === 0 &&
                    // 2. ローカルにReazonSpeechモデルがダウンロードされていないか、されている場合であっても話し手の言語が日本語以外の場合
                    (downloadedModelsReazonSpeech.length === 0 || downloadedModelsReazonSpeech.length > 0 && !(speakerLanguage?.startsWith("ja") || speakerLanguage?.startsWith("small-ja"))) &&
                    // 3. WhisperのAPIキーが設定されていない場合
                    settingKeyOpenai === "" &&
                    // 4. ローカルにWhisperLargeモデルがダウンロードされていない場合 or ローカルにFugumtEnJaモデルがダウンロードされていない場合 or ダウンロードされていても、話し手の言語が日本語の場合
                    ...
                    // 5. ローカルにWhisperLargeモデルがダウンロードされていない場合 or ローカルにFugumtJaEnモデルがダウンロードされていない場合 or ダウンロードされていても、話し手の言語が日本語以外の場合
            ...
            >
                <ul className="max-h-56 overflow-y-scroll rounded-box scrollbar-transparent">
                    ...
                    {settingKeyOpenai
                        ...
                        && downloadedModelsReazonSpeech.length > 0
                        ...
                        <>
                            ...
                            <div className="flex justify-center sticky top-0 z-10 w-full" style={{ backgroundColor: "hsl(24 33% 97%)" }}>
                                <p className="text-xs text-gray-400">ハイブリッド</p>
                            </div>
                            ...
                            <li key="transcription-accuracy_hybrid-transcript">
                                <label className="label inline-flex active:!bg-inherit">
                                    <input type="radio" name="trace-option" className="radio radio-accent" onChange={change} value="hybrid-transcript" checked={"hybrid-transcript" === transcriptionAccuracy} />
                                    <a className="grow">Lycoris</a>
                                </label>
                            </li>
                        </>}
                    {(downloadedModels.length > 0
                        || (downloadedModelsReazonSpeech.length > 0 && (speakerLanguage?.startsWith("ja") || speakerLanguage?.startsWith("small-ja"))))
                        && <>
                            <hr className="my-2" />
                            <div className="flex justify-center sticky top-0 z-10 w-full" style={{ backgroundColor: "hsl(24 33% 97%)" }}>
                            ...
                            {[...downloadedModels, ...downloadedModelsReazonSpeech]
                                ...
                                .reduce((a: string[], c) => {
                                    ...
                                    if (c === "large-distil.ja" || c === "reazonspeech") {
